...
func TestCompleteLatestClassificationOperationsDoNotTruncate(t *testing.T) {
	s := newStore(t)
	ctx := context.Background()
	if err := s.WithTx(ctx, func(tx *Tx) error {
		for i := 0; i < 1001; i++ {
			if err := tx.InsertClassification(ClassificationRecord{
				ProcUID: fmt.Sprintf("%d:1", i+10), SessionID: "s1", TsNs: int64(i + 1),
				ActivityTsNs: int64(i + 1), State: "idle", BasisState: "idle",
				StableSinceNs: 1, EvidenceJSON: "[]",
			}); err != nil {
				return err
			}
		}
	...
	latest, err := s.LatestClassifications(ctx, "s1")
	if err != nil || len(latest) != 1001 {
		t.Fatalf("complete latest set = %d records, %v", len(latest), err)
	}
	counts, err := s.ClassificationCounts(ctx, "s1")
	if err != nil || counts["idle"] != 1001 {
		t.Fatalf("complete counts = %+v, %v", counts, err)
	}
}

...
func (r RetentionResult) Total() int64 {
	return r.Observations + r.ActivitySamples + r.Classifications + r.PolicyDecisions + r.PolicyEvaluations +
		r.Actions + r.WorktreeActions + r.WorktreeMissing + r.WorktreeTombstones +
		r.Scans + r.Audit + r.Processes + r.Ownership + r.Sessions + r.Relationships +
...
func (s *Store) compactOnce(ctx context.Context, p RetentionPolicy, now time.Time, res *RetentionResult) error {
	...
	return s.WithTx(ctx, func(t *Tx) error {
		steps := []struct {
			...
			{&res.WorktreeActions, `DELETE FROM worktree_actions WHERE requested_ns < ? AND result NOT IN ('attempting', 'purging', 'partial')`, []any{cutoff(p.Actions)}},
			{&res.WorktreeMissing, `DELETE FROM worktrees WHERE registered = 0 AND state NOT IN ('removed', 'retired')
				AND last_seen_ns < ? AND worktree_id NOT IN (
					SELECT worktree_id FROM worktree_actions WHERE result IN ('attempting', 'purging', 'partial')

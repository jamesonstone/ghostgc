...
func renderMetrics(m api.MetricsResponse) {
	...
	rows := [][2]string{
		...
		{"database", fmt.Sprintf("%s (%d sessions, %d processes, %d observations, %d edges, %d audit)",
			humanBytes(uint64(m.DatabaseBytes)), m.DatabaseCounts.Sessions, m.DatabaseCounts.Processes,
			m.DatabaseCounts.Observations, m.DatabaseCounts.Relationships, m.DatabaseCounts.AuditEntries)},
